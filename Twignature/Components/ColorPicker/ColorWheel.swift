...
open class ColorWheel: UIView {
    ...
    public init(frame: CGRect, color: UIColor!) {
        ...
        brightnessLayer.path = UIBezierPath(roundedRect: CGRect(x: 0.5, y: 0.5, width: self.frame.width-0.5, height: self.frame.height-0.5), cornerRadius: (self.frame.height-0.5)/2).cgPath
    ...
    func pointForColor(_ color: UIColor) -> CGPoint {
        var hue: CGFloat = 0.0, saturation: CGFloat = 0.0, brightness: CGFloat = 0.0, alpha: CGFloat = 0.0
        let ok: Bool = color.getHue(&hue, saturation: &saturation, brightness: &brightness, alpha: &alpha)
        if (!ok) {
            print("SwiftHSVColorPicker: exception <The color provided to SwiftHSVColorPicker is not convertible to HSV>")
        }
        return pointAtHueSaturation(hue, saturation: saturation)
    }
